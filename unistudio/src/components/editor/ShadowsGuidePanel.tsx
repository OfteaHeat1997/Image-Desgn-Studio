...
interface ShadowsGuidePanelProps {
  hasImage: boolean;
  hasProcessedImage: boolean;
  processedImageUrl: string | null;
  sessionResults: SessionResult[];
  selectedShadowType: string;
}
...
const GUIDE_STEPS = [
  { id: "upload", text: "Sube una imagen de producto" },
  { id: "type", text: "Elige un tipo de sombra" },
  { id: "adjust", text: "Ajusta los controles" },
  { id: "apply", text: 'Haz clic en "Aplicar Sombra"' },
  { id: "compare", text: "Compara el antes y despues" },
  { id: "download", text: "Descarga el resultado" },
];

const PRO_TIPS = [
  "Usa imagenes PNG con fondo transparente para mejores resultados.",
  "Las opciones gratis (Sombra Suelta, Contacto, Reflejo) no usan IA y son instantaneas.",
  'Puedes encadenar resultados: aplica una sombra, haz clic en "Aceptar y Seguir Editando", y aplica otra.',
];
...
/* ------------------------------------------------------------------ */
/*  Shadow type labels (for history)                                    */
/* ------------------------------------------------------------------ */

const SHADOW_LABELS: Record<string, string> = {
  drop: "Sombra Suelta",
  contact: "Sombra de Contacto",
  reflection: "Reflejo",
  "ai-relight": "Iluminacion IA",
  "ai-kontext": "Sombras IA",
};
...
export function ShadowsGuidePanel({
  hasImage,
  hasProcessedImage,
  processedImageUrl,
  sessionResults,
  selectedShadowType,
}: ShadowsGuidePanelProps) {
  ...
  if (hasImage) {
    ...
    completedSteps.add("type"); // always selected by default
  }
  if (hasImage) completedSteps.add("adjust");
  if (hasProcessedImage) {
    completedSteps.add("apply");
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `resultado-sombra.${downloadFormat}`;
      ...
      <div className="p-3">
        ...
        {!hasProcessedImage ? (
          <p className="text-[11px] text-gray-600 text-center py-2">
            Aplica una sombra primero para poder descargar.
          </p>
        ) : (
...
export default ShadowsGuidePanel;
