...
function buildFallbackPlan(req: AgentPlanRequest): AgentPlan {
  ...
  switch (req.agentType) {
    ...
    case "social":
      ...
      break;
    // "catalogo" and "cambiar-modelo" agent types were removed in commit 8 of
    // the pipeline rewrite — they duplicated /pipelines/lingerie. Anything that
    // used to request them now falls through to the default branch.
    default:
      steps = getEcommercePipeline(req.productCategory, budget);
      name = "Agente E-Commerce";
