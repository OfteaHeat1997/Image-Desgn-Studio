...
import { Sparkles, Zap, Monitor, Cpu, Wand2, Image as ImageIcon, RefreshCw } from "lucide-react";
...
// Fix 2 — product type options
const PRODUCT_TYPE_OPTIONS = [
  { value: "product", label: "General" },
  { value: "lingerie", label: "Lenceria" },
  { value: "fragrance bottle", label: "Fragancias" },
  { value: "jewelry piece", label: "Joyeria" },
  { value: "skincare product", label: "Skincare" },
];
...
// Fix 1 — expose creative mode
type ApiMode = "precise" | "fast" | "creative";

const API_MODE_OPTIONS: { value: ApiMode; label: string; desc: string; cost: string }[] = [
  { value: "precise", label: "Preciso", desc: "Preserva producto exacto", cost: "$0.05" },
  { value: "fast", label: "Rapido", desc: "Composicion directa", cost: "$0.003" },
  { value: "creative", label: "Creativo", desc: "Genera escena desde descripcion", cost: "$0.03" },
];
...
export function BgGeneratePanel({ imageFile, onProcess }: BgGeneratePanelProps) {
  ...
  const isCreative = apiMode === "creative";
  const isFree = !!preset?.color && !customPrompt && !isCreative;

  const handleGenerate = useCallback(async () => {
    // Creative mode doesn't need an image file
    if (!isCreative && !imageFile) return;
    ...
    try {
      if (isFree && preset?.color) {
        ...
        // ---- CREATIVE MODE — text only, no image upload ----
        let optimizedPrompt = creativeDesc;

        if (creativeDesc) {
          setStatusText("Claude IA optimizando tu descripcion...");
          setProgressPct(20);
          try {
            ...
            console.warn("[bg-generate] Claude optimization failed, using raw description");
          }
        }

        setStatusText("Generando escena con IA (Replicate)...");
        setProgressPct(50);

        // Fix 5 — simulate progress after 50%
        ...
        if (!res.ok) {
          ...
          const msg = text.startsWith("{") ? JSON.parse(text).error : `Error del servidor (${res.status})`;
          ...
          try {
            const promptRes = await fetch("/api/prompt", {
              ...
              body: JSON.stringify({
                ...
                context: {
                  productType, // Fix 2 — dynamic product type
                  brandStyle: "luxury e-commerce",
                  desiredMood: "elegant",
                },
              }),
      ...
      setStatusText("");
      // Fix 4 — detect 429 errors
      const err = error as Error & { status?: number };
      const is429 = err.status === 429 || err.message?.includes("429");
      setErrorMsg(is429 ? "Servidor ocupado. Espera un momento e intenta de nuevo." : (err.message || "Error al generar el fondo"));
    } finally {
  ...
  }, [imageFile, isFree, isCreative, preset, selectedPreset, customPrompt, creativeDesc, aspectRatio, apiMode, productType, onProcess]);

  const activeCategoryPresets = STYLE_PRESETS.filter((p) => p.category === activeCategory);
  const activeCatIsFree = activeCategoryPresets.some((p) => !!p.color);
    ...
    <div className="space-y-4">
      ...
      <div className={cn(
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[9px] text-gray-500">
            {isFree
              ...
              : isCreative
                ? "Costo estimado: ~$0.03 por escena"
        ...
        <span className={cn(
          ...
          {isFree ? "GRATIS" : isCreative ? "~$0.03" : apiMode === "fast" ? "~$0.003" : "~$0.05"}
        </span>
      </div>

      {/* API Mode selector (only visible when not free) — Fix 1: 3 cols, creative included */}
      {!isFree && (
        <div>
          <label className="mb-2 block text-xs font-medium text-gray-400">
            Modo de Generacion
      ...
      {/* Fix 1 — Creative mode: show description textarea instead of preset/image */}
      {isCreative ? (
        <div className="rounded-lg border border-purple-500/30 bg-purple-500/5 p-3 space-y-2">
          ...
          <p className="text-[9px] text-gray-500">
            Describe la escena completa que quieres generar. No necesitas subir una imagen — la IA crea todo desde tu descripcion.
          </p>
          <textarea
            value={creativeDesc}
            onChange={(e) => setCreativeDesc(e.target.value)}
            placeholder="ej. frasco de perfume sobre marmol negro con rosas rojas y luz dorada suave, fotografia de lujo..."
            rows={3}
          ...
          <p className="text-[9px] text-purple-400">
            Usa Replicate API (~$0.03) + Claude IA para crear tu escena unica desde cero.
          </p>
        </div>
      ) : (
        <>
          {/* Fix 3 — Category tabs instead of scroll */}
          <div>
            <label className="mb-2 block text-xs font-medium text-gray-400">
              Elige un Fondo
            </label>
            ...
            {/* Free badge for current category */}
            {activeCatIsFree && (
              <p className="mb-1.5 text-[9px] text-emerald-400 font-medium">
                GRATIS — se procesa en tu navegador
              </p>
            )}

            {/* Preset grid for active category */}
            <div className="rounded-lg border border-surface-lighter bg-surface p-2">
              <div className="grid grid-cols-4 gap-1">
                {activeCategoryPresets.map((p) => (
                  <button
                    key={p.id}
                    type="button"
      ...
      {/* Fix 5 — Progress bar with simulated fill */}
      {isProcessing && (
      ...
      <Button
        ...
        disabled={(!imageFile && !isCreative) || (isCreative && !creativeDesc.trim()) || isProcessing}
