...
| tryon (dedicado, opt-in) | **Uwear.ai** (`api.uwear.ai`) | Plataforma de fotografía de moda. Genera SU propia modelo (virtual o avatar guardado) vistiendo un "clothing item" registrado desde las fotos del producto (frente + espalda). Permite lencería explícitamente vía **SeedDream 4.5** (`seedream-v4-5`, default) y **Qwen Intimate** (`qwen-rapid-aio-v23`, requiere workspace verificado). Flujo: `POST /clothing-item` → `POST /generation` → poll `GET /generation/{id}`. Cliente en `src/lib/api/uwear.ts`, requiere `UWEAR_API_KEY` en Vercel. Ignora la `modelImage` de UniStudio (Uwear castea su propia modelo). `model_slug` overrideable con `UWEAR_MODEL_SLUG`. ~$0.20 (2 créditos × $0.10). Validado a mano con el bra de soporte: respeta textura/costuras/malla. |
